...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			%d)
			\item[\textbf{d)}] $z_1 = j \text{, } z_2=-2-4j$
			\begin{align*}
			z_1+z_2 &= -2+j-4j \\
					&= -2-3j \\\\
			z_1-z_2 &= j+2+4j \\
			...
			z_1 \cdot z_2 &= j(-2-4j) \\
			              &= 4-2j\\\\
			z_1 / z_2 &= \frac{j}{(-2-4j)} \\
			          &= \frac{-4}{4+16}+j\frac{-2}{4+16} \\
			          &= -\frac{-1}{5}-j\frac{1}{10} \\\\
			z_1^* \cdot z_2 &= -j(-2-4j) \\
			              	&= -4+2\\\\
			z_1 / z_2^* &= \frac{j}{-2+4j}\\
						&= \frac{4}{4+16} + j\frac{-2}{4+16} \\
		...
		\end{enumerate}
		\item[\textbf{2)}]
		$e^{i\theta}$ stellt einen Vektor in der komplexen Zahlenebene da, der um einen Winkel $\theta$ um den Einheitskreis rotiert worden ist. Mit der eulerschen Formel $e^{i\theta} = \cos(\theta)+i\sin(\theta)$ bekommt man die kartesischen Koordinaten des rotierten Vektors. Da $\pi$ genau eine halbe Rotation um den Einheitskreis ist bekommt man $e^{i\pi} = -1$. Dies kann man dann noch umformen um Eulers Identität $e^{i\pi} +1 = 0$ zu erhalten.
